...
public class OverlayRenderer implements Renderer {
	...
	private void renderDebugText(SpriteBatch batch, Camera camera) {
		...
		debugLine(batch, camera, line++, String.format("World: X:%.0f Y:%.0f", WorldUtil.colRowToWorldCords(colRow[0], colRow[1])[0], WorldUtil.colRowToWorldCords(colRow[0], colRow[1])[1]));
		line++;
